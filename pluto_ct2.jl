...
# ╔═╡ 0536974e-02cb-4905-9c95-32be3adfeaca
md"""
θ = $(@bind a2 Slider(0:1:179, show_value=true, default=0)) 
"""

# ╔═╡ 1c501345-e920-46ec-8503-6ca424a44270
md"""
#### Example of X-ray transform on $\mathbb{R}^2$
"""
...
$nbsp $nbsp $nbsp
t = $(@bind b1 Slider(-200:1:200, show_value=true, default=0))
"""
...
begin
	if a1==90
		...
		P=(1,220+220*tan(pi*a1/180));
		Q=(440,220-220*tan(pi*a1/180));
		R=(220+b1/cos(pi*a1/180)-220*tan(pi*a1/180),1);
		S=(220+b1/cos(pi*a1/180)+220*tan(pi*a1/180),440);
end
    ...
    p=plot(Gray.(z),
         ...
         ylabel="y");
	  segment!(P,Q,color=:lightgreen,width=4,legend=false); 
...
# ╔═╡ c961036d-a14f-47b8-9bd3-ca89c1e5ef32
begin
	c0=-200;
	c1=-100;
	c2=0;
	...
	if a2==90
		...
		R20=(220+c0/cos(pi*a2/180)-220*tan(pi*a2/180),1);
		S20=(220+c0/cos(pi*a2/180)+220*tan(pi*a2/180),440);
		R21=(220+c1/cos(pi*a2/180)-220*tan(pi*a2/180),1);
		S21=(220+c1/cos(pi*a2/180)+220*tan(pi*a2/180),440);
		R22=(220+c2/cos(pi*a2/180)-220*tan(pi*a2/180),1);
		S22=(220+c2/cos(pi*a2/180)+220*tan(pi*a2/180),440);
		R23=(220+c3/cos(pi*a2/180)-220*tan(pi*a2/180),1);
		S23=(220+c3/cos(pi*a2/180)+220*tan(pi*a2/180),440);
		R24=(220+c4/cos(pi*a2/180)-220*tan(pi*a2/180),1);
		S24=(220+c4/cos(pi*a2/180)+220*tan(pi*a2/180),440);
	end
    ...
    p2=plot(Gray.(z),
         ...
         yticks = ([1 w/2  w],[floor(w/2),0,-floor(w/2)]),
         ylabel="y")
　      segment!((220-210*cos(pi*a2/180),220+210*sin(pi*a2/180)),(220+210*cos(pi*a2/180),220-210*sin(pi*a2/180)),color="lightgreen",width=4,legend=false);
     	segment!(R20,S20,color=:magenta,width=2,legend=false);
        segment!(R21,S21,color=:magenta,width=2,legend=false);
        segment!(R22,S22,color=:magenta,width=2,legend=false);
        segment!(R23,S23,color=:magenta,width=2,legend=false);
        segment!(R24,S24,color=:magenta,width=2,legend=false);
...
# ╔═╡ 3b12c09a-1d91-4f41-bcec-75bc837c1cf8
begin
	q2=plot(sinogram[:,a2+1],
	        size=(600,400), 
            title="Slice of Sinogram (X-ray transform)", 
            xaxis="t", 
		    xticks = ([112 212 312 412 512], [-200,-100,0,100,200]),
            yaxis="R_1f(θ,*)",
	        legend=:false,
	        color=:skyblue,
	        width=4)
...
# ╔═╡ bd325028-8239-41cb-8a85-7c02b1db76d8
begin
	plot(p2, q2,
         layout=2, 
         size=(1400,400), 
...
# ╔═╡ 51e4a92a-b166-4b67-87bf-ee1c3f337500
begin
size(sinogram)
end
...
manifest_format = "2.0"
project_hash = "7dcafde279788d78d4397ffecfd0d3a10081b8b6"
