...
# ╔═╡ bae785ec-b8d5-11ee-2509-754582ab7cee
begin
	using Plots
    using DSP
    using Wavelets
...
# ╔═╡ 06ffcace-018f-4578-9275-c7a5d834c590
md""" ### Signal Processing

"""

# ╔═╡ cc2d16bf-9a3d-477f-93c1-ee9756ee2ae1
begin
	# audio signal
    w=load("./material/cheese.wav");
    fs=w.samplerate
    x=w.data[:, 1];
...
# ╔═╡ 53d3c787-4aa0-4a11-99cd-3e546ef51171
md""" ### Sound Data
"""

# ╔═╡ 1aefee54-24ca-4629-99d8-25ea26d7523e
begin
	# analogue signal 
p1=plot(t,x,
    xaxis="time \$t\$ [sec]", 
...
plot(p2,size=(600,400))

end

# ╔═╡ 77cb1d08-7a8b-4cd0-afbe-a7510f39f6db
begin
	hats=fft(x);
...
# ╔═╡ e025f238-ac79-4ea4-aa57-76304a2ee273
begin
	print("N=",length(x));
end

# ╔═╡ 2d03ff5e-3d88-4a63-af5d-9192c7a56e33
md""" ### Discrete Fourier Transform
"""

# ╔═╡ 28866ef3-8c51-4c5f-9b91-13605b57bb56
begin
	plot(G,
    xaxis=L"k = 0, 1, 2, ..., N-1", 
    yaxis=L"|F(x)[k]|", 
...
# ╔═╡ 53437ffe-f323-4c5d-b799-4f93e1760e8a
md""" ### Sampled Discrete Fourier Transform
"""

# ╔═╡ fe53b769-b66d-4755-9f02-3a85b6974a8f
begin
	plot(k,G,
    xaxis="frequency \$k\$", 
    yaxis=L"|F(x)[k]|", 
...
# ╔═╡ 7c3b3981-26f1-4322-a24c-e8a1c3bf38d5
begin
...
# ╔═╡ abcd40ee-def4-4e61-b725-f8f70b223b1f
md""" ### Shifted Discrete Fourier Transform

In some sense this is a discrete version of the Fourier transform of a functions of one variable on $\mathbb{R}$: 

$\hat{f}(\xi)=\int_{-\infty}^\infty e^{-2\pi ix\xi} f(x)dx$
"""

# ╔═╡ 3832608e-72f7-4275-a7f9-a26e519bbd15
begin
	# sampled FFT shifted
...
# ╔═╡ 64f348a5-8ccc-483d-8874-5faf19a5ad8f
begin
	# setting integers
	L=256;
	M1=128;
	M=2*M1;
	N0=250;
	n0=1+floor(Int, (length(x)-L)/(L-N0));
	s=L-N0;
...
# ╔═╡ 0e3db096-4f04-4e2a-9de5-e5c7188bd479
begin
...
# ╔═╡ 8fedc41f-62b2-4426-a280-d1a9097c3153
md""" ### Spectrogram

This is a discrete version of a short-time Fourier transform with a window function $g\ne0$:

$V_gf(x,\xi)=\int_{-\infty}^\infty e^{-2\pi ix\xi} \ \overline{g(y-x)}\ f(y)dy.$
"""

# ╔═╡ f167b107-4fd9-492d-90bc-40d3f7411f3c
begin
	#  plotting
p5=plot(X1, 
...
# ╔═╡ 186da8d2-c1b6-4533-9c7b-238f39b804c1
md""" ### Wavelet Decomposition
"""

# ╔═╡ 27379ef5-c92d-4603-bc24-7597c44ec402
begin
	y=zeros(length(x),4);
	ya=zeros(length(x),4);
	yd=zeros(length(x),4);
	...
	for j=1:4
		y[:,j]=dwt(X[:,2], wavelet(WT.haar), j);
		ya[1:Int(length(x)/2^j),j]=y[1:Int(length(x)/2^j),j];
		yd[:,j]=y[:,j]-ya[:,j];
		za[:,j+1]=idwt(ya[:,j], wavelet(WT.haar), j);
		zd[:,j+1]=idwt(yd[:,j], wavelet(WT.haar), j);
	end
	
end

# ╔═╡ 913fe339-973f-4e27-abcf-49e50e6337aa
md"""
level = $(@bind level Slider(0:4, show_value=true))
"""

# ╔═╡ fd021e15-c061-4d87-ae5d-917412700460
begin
# original
p11=plot(x, 
    xaxis="time \$t\$ [sec]", 
...
plot!(yticks = ([-1 -0.5 0 0.5 1;], [-1,-0.5,0,0.5,1]))
...
manifest_format = "2.0"
project_hash = "7054befffd0ca275a572f30e8e226c7e8f33c3c5"
