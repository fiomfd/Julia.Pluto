...
begin 
	z = zeros(800,800);
    for i=151:650
        for j=151:150+Int64(ceil(sqrt(250000-(650-i)^2)))
            z[i,j]=1;
        end
    end
...
# ╔═╡ eb03dda3-dc86-4a03-98b5-590337d828a4
begin
	include("xray.jl")
...
# ╔═╡ 54359420-226b-4271-aaf6-f24a54ebe14d
begin
	(a,b)=size(sinogram);
    p=Int64(floor(w-2));
    q=Int64(floor(h-2));
...
# ╔═╡ cf9d34a7-b1f5-4440-81b1-bc4062224f9b
begin
    fil=zeros(Complex, a);
	hann=ones(a)-hanning(a);
	for k=1:a
	    fil[k]=hann[k]*min(k-1,a+1-k);
	end
	...
	for m=1:a
	    for n=1:a
		    for k=1:a
			    K[m,n]=K[m,n]+2*pi*fil[k]*omega^((m-n)*(k-1))/(a^2);
			end
		end
	end
    ...
    Uf=zeros(Complex, p,q,b);
	Vf=zeros(Complex, p,q);

    for i=1:p
        for j=1:q
            for k=1:b
                Uf[i,j,k]=SINOGRAM[Int64(t[i,j,k]),k]/2+SINOGRAM[Int64(s[i,j,k]),k]/2;
             end
             Vf[i,j]=sum(Uf[i,j,:])/(2*b);
         end
    end
...
# ╔═╡ 94acdb5d-b463-4c82-9ae0-e98fdec34fe7
begin
	p1=plot(Gray.(z),
...
#   right_margin=Plots.Measures.Length(:mm, 10.0),
            ...
            ylabel=L"y")
    savefig("./output/xray_quater_1.png") 
    ...
    savefig("./output/xray_quater_2.png")
...
#    right_margin=Plots.Measures.Length(:mm, 10.0),
            ...
            ylabel=L"y")
    savefig("./output/xray_quater_3.png") 
...
#    right_margin=Plots.Measures.Length(:mm, 10.0),
    ...
    　　　　　yticks = ([p/2-250 p/2  p/2+250;], [1,0,-1]),
    　　　　　ylabel=L"y")
　　　savefig("./output/xray_quater_4.png") 
...
# ╔═╡ 42f3d7e2-b265-4a05-a52b-5cbeea5e8f1b
md""" ### 2-Dimensional Computer Tomography
...
$\mathcal{X}f(\gamma)=\mathcal{X}f(\theta,t)
:=\int_\gamma f=\int_{-\infty}^\infty f(t\cos\theta-s\sin\theta,t\sin\theta+s\cos\theta)ds.$

We have $\mathcal{X}f(\theta+\pi,-t)=\mathcal{X}f(\theta,t)$ for $(\theta,t)\in[0,\pi]\times\mathbb{R}$. 

For $F(\theta,t) \in C^\infty([0,2\pi]\times\mathbb{R})$ with $F(\theta+\pi,-t)=F(\theta,t)$ for $(\theta,t)\in[0,\pi]\times\mathbb{R}$, the adjoint of $\mathcal{X}$ is given by 

$
\begin{aligned}
  \mathcal{X}^TF(x,y)
& :=
  \frac{1}{4\pi} \int_0^{2\pi} F(\theta,x\cos\theta+y\sin\theta)d\theta
\\
& =
  \frac{1}{2\pi} \int_0^\pi F(\theta,x\cos\theta+y\sin\theta)d\theta.
\end{aligned}$

For an appropriate function $f(x,y)$, $\mathcal{X}^T\mathcal{X}f(x,y)$ is said to be the unfiltered back-projection, and $(-\partial_x^2-\partial_y^2)^{1/2}\mathcal{X}^T\mathcal{X}f(x,y)$ is said to be the filtered back-projection. It is well-known that the fintered back-projection formula

$f(x,y)=(-\partial_x^2-\partial_y^2)^{1/2}\mathcal{X}^T\mathcal{X}f(x,y)$

holds.
"""

# ╔═╡ 7f7e7fae-4617-4add-84e4-8820be8743b5
begin
	plot(p1,p2,p3,p4,
		layout=4, 
		size=(1000,800), 
    ...
    savefig("./output/xray_quater_5.png") 

end

# ╔═╡ a1489a7f-9ed6-4cea-9320-6d8fd5dfa014
# ╠═╡ disabled = true
#=╠═╡
begin
	sinogram1=zeros(1131,180,6);

	for j=31:180
		sinogram1[:,j,1]=sinogram[:,j];
		sinogram1[:,181-j,6]=sinogram[:,181-j];
	end
	for j=1:30
		sinogram1[:,j,2]=sinogram[:,j];
		sinogram1[:,181-j,5]=sinogram[:,181-j];
	end
	for j=61:180
		sinogram1[:,j,2]=sinogram[:,j];
		sinogram1[:,181-j,5]=sinogram[:,181-j];
	...
	for j=1:60
		sinogram1[:,j,3]=sinogram[:,j];
		sinogram1[:,181-j,4]=sinogram[:,181-j];
	end
	for j=91:180
		sinogram1[:,j,3]=sinogram[:,j];
		sinogram1[:,181-j,4]=sinogram[:,181-j];
...
# ╔═╡ e1edea92-a5e9-4ab4-9b5c-53216f2b7596
# ╠═╡ disabled = true
#=╠═╡
begin
	q2=plot(sinogram1[:,:,step], 
		    clim=(0,710),
            size=(600,400), 
            title="Limited Angle: X-ray transform \$Xf(θ,t)\$", 
            st=:heatmap, 
            color=:jet1, 
            xaxis="\$θ\$ [rad]", 
            yaxis=L"t")
    plot!(xticks = ([1 90 180;], [0,"π/2","π"]))
    plot!(yticks = ([Nr/2-250 Nr/2 Nr/2+250;], [-1,0,1]))
	#savefig("./output/xray_quater_6.png") 
...
# ╔═╡ aaab60cf-3a1c-4892-85f0-9c22860bb0ee
# ╠═╡ disabled = true
#=╠═╡
begin
	SINOGRAM1=zeros(Complex, a, b, 6);
	for k=1:6
        for l=1:b
        SINOGRAM1[:,l,k]=K*sinogram1[:,l,k];
		end
    end
...
# ╔═╡ 4ab215a1-8c81-4367-8bf1-cab6a9370631
# ╠═╡ disabled = true
#=╠═╡
begin
	U1=zeros(Complex, p,q,b,6);
	V1=zeros(Complex, p,q,6);
	
	for i=1:p
        for j=1:q
            for l=1:b
                U1[i,j,l,1]=SINOGRAM1[Int64(t[i,j,l]),l,1]/2+SINOGRAM1[Int64(s[i,j,l]),l,1]/2;

            end
             V1[i,j,1]=sum(U1[i,j,:,1])/(2*b);
		end
	end
	...
	for i=1:p
        for j=1:q
            for l=1:b
                U1[i,j,l,2]=SINOGRAM1[Int64(t[i,j,l]),l,2]/2+SINOGRAM1[Int64(s[i,j,l]),l,2]/2;

            end
             V1[i,j,2]=sum(U1[i,j,:,2])/(2*b);
		end
	end
...
# ╔═╡ 521d51c0-5489-4183-b4c6-cae22240359c
# ╠═╡ disabled = true
#=╠═╡
begin
	q4=plot(Gray.(W1[:,:,step]),
    　　　　　title="Filtered Back-Projection", 
#    right_margin=Plots.Measures.Length(:mm, 10.0),
    　　　　　size=(400,400), 
    　　　　　xticks = ([p/2-250 p/2  p/2+250;], [-1,0,1]),
    ...
    　　　　　yticks = ([p/2-250 p/2  p/2+250;], [1,0,-1]),
    　　　　　ylabel=L"y")
　　　#savefig("./output/xray_quater_7.png") 
...
# ╔═╡ f4872b97-d0b9-4cc9-b586-e114b86e4dd3
md""" ### Limited Angle Tomography
For $k=1,2,3,4,5,6$ 

$\mathcal{X}_kf(\theta,t):=\mathcal{X}f(\theta,t), 
\quad 
\theta\not\in[\pi(k-1)/6,\pi{k}/6],$

$\mathcal{X}_kf(\theta,t):=0, 
\quad 
\theta\in[\pi(k-1)/6,\pi{k}/6],$

$f_k(x,y):=(-\partial_x^2-\partial_y^2)^{1/2}\mathcal{X}^T\mathcal{X}_kf(x,y).$

"""

# ╔═╡ 353e7a40-683b-44d6-8583-aaacdc7029c2
# ╠═╡ disabled = true
#=╠═╡
md"""
k = $(@bind step Slider(1:1:6, show_value=true))
"""
  ╠═╡ =#

# ╔═╡ 79454351-2702-4b1c-9df6-5645168ab213
# ╠═╡ disabled = true
#=╠═╡
begin
	plot(p1,p2,q2,#q4,
...
manifest_format = "2.0"
project_hash = "8635c5fb7e8ecaaae88dac18d48aeae7eba0f737"
