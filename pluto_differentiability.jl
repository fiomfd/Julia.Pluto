...
$\frac{f(x)-f(a)}{x-a} \rightarrow \alpha \quad (x \rightarrow a).$
...
$f(x)=f(a)+\alpha(x-a)+R(x), \quad \frac{R(x)}{x-a}=\frac{f(x)-f(a)}{x-a} - \alpha.$

Then $f(x)$ is differentiable at $x=a$ if and only if there exist $\alpha\in\mathbb{R}$ and a function $R(x)$ such that 

$f(x)={\color{blue}{f(a)+\alpha(x-a)}}+R(x), \quad {\color{red}{\frac{R(x)}{x-a} \rightarrow 0}} \quad (x \rightarrow a).$
...
md""" 
## About $\quad {\color{red}{R(x)/(x-a) \rightarrow 0}} \quad (x \rightarrow a)$

The condition ${\color{red}{\dfrac{R(x)}{x-a} \rightarrow 0}} \quad (x \rightarrow a)$ means that $R(x)$ goes to $0$ as $x \rightarrow a$ faster than the linear funtion of $x-a$.  
...
# ╔═╡ a488a14c-68b0-45d7-9b31-2aef8f394471
md""" 
## All the lines passing through $\bigl(a,f(a)\bigr)$
Consider lines given by the graph of functions

$g(x)={\color{darkorange}{f(a)+\beta(x-a)}}, \quad \beta\in\mathbb{R}, \qquad h(x)={\color{green}{f(a)+f^\prime(a)(x-a)}}.$ 

 $h(x)$ is the special case of $g(x)$ with $\beta=f^\prime(a)$. Consider how fast the differences $f(x)-g(x)$ and $f(x)-h(x)$ go to $0$ as $x \rightarrow a$.  

- If $\beta \ne f^\prime(a)$ then as fast as $x-a$. 
$f(x)-g(x)={\color{magenta}{\bigl(f^\prime(a)-\beta\bigr)(x-a)}} + {\color{red}{R(x)}}.$

- If $\beta = f^\prime(a)$ then faster than $x-a$. 
$f(x)-h(x)={\color{red}{R(x)}}.$

So the line given by $y=h(x)$ is a special line for $y=f(x)$ at $\bigl(a,f(a)\bigr)$. The line given by $y=h(x)$ is said to be the tangential line of $y=f(x)$ at $\bigl(a,f(a)\bigr)$. Roughly spaeking, we say that $f(x)$ is differentiable at $x=a$ if there exists a unique tangential line of $y=f(x)$ at $\bigl(a,f(a)\bigr)$.
"""

# ╔═╡ 76cb2ec2-fc76-4ad5-8d57-777395755f7f
md"""
#### Example $\quad f(x)=x^2$  
$g(x)={\color{darkorange}{a^2+\beta(x-a)}}, 
\quad 
h(x)={\color{green}{a^2+2a(x-a)}}.$

$f(x)-g(x)={\color{magenta}{(2a-\beta)(x-a)}}+{\color{red}{(x-a)^2}}, \quad f(x)-h(x)={\color{red}{(x-a)^2}}.$

"""

# ╔═╡ 092ba07e-4866-40c6-ade8-25d9c2218011
md"""
a = $(@bind a1 Slider(-4:0.1:4, show_value=true, default=1)) 
...
begin
    plot([f1,g1,h1],grid=false,
        ...
        title=L"y=f(x), y=f(a)+\beta(x-a), y=f(a)+f'(a)(x-a)",
        xticks = :false,
		xlabel=L"x",
	    label = [L"y=f(x)" L"y=f(a)+\beta(x-a)" L"y=f(a)+f'(a)(x-a)"],
		ylabel=L"y",
		yticks=:false,
	    legend = :top)
...
manifest_format = "2.0"
project_hash = "9793ca78f1e2d02275c00d38716f569286f70aab"
