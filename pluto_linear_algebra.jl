...
md""" #### Linear Algebra of Numeric Vectors
1. Matrices and numeric vectors

2. Data described by matrices
...
md"""
##### 1-1. Definition of matrices and numeric vectors

Let $m$ and $n$ be positive integers. Given $A=[a_{ij}]$ is said to be an 
$m{\times}n$ matrix if $mn$ (real) numbers 

$a_{ij}, \quad i=1,\dotsc,m, \quad j=1,\dotsc,n$ 

are given, arranged in the form of $m$ rows and $n$ columns, and enclosed by $[\quad]$ or $(\quad)$: 
...
are matrices. If $m$ or $n$ is equal to one, 
...
are said to be $m$-dimensional column vector and $n$-dimensional row vector respectively. If $m=n=1$, $[a_{11}]$ is idenfified as the scalar $a_{11}$.  
"""

# ╔═╡ b4bcd9fb-1960-445d-bde1-5755fd5662e8
md"""
##### 1-2. Operations of matrices and numeric vectors

- For an $m{\times}n$ matrix $A=[a_{ij}]$, we define its transpose $A^T$, which is an  $n{\times}m$ matrix, by:
$A^T:=[a_{ji}], 
\quad 
\begin{bmatrix}
1 & 2 & 3
...
- For $m{\times}n$ matrices $A=[a_{ij}]$ and $B=[b_{ij}]$, we define their sum $A+B$, which is also an $m{\times}n$ matrix, by:
$A+B:=[a_{ij}+b_{ij}],$
$\begin{bmatrix}
1 & 2 & 3
\\
...
- Foa a matrix $A=[a_{ij}]$ and a scalar $cA$, we define a scalar product $cA$ by:
$cA:=[ca_{ij}],
\quad
5
\begin{bmatrix}
...
- Let $\vec{a}_1,\dotsc,\vec{a}_n$ be $m$-dimensional column vectors, and 
$A$ be an $m{\times}n$ matrix defined by $A:=\begin{bmatrix}\vec{a}_1 & \dotsb & \vec{a}_n\end{bmatrix}$. For an $n$-dimensional column vector $\vec{x}=\begin{bmatrix}x_1 \\ \vdots \\ x_n\end{bmatrix}$, we define the $m$-dimensional column vector $A\vec{x}$ by:
...
- Let $A$ be an $m{\times}n$ matrix, and let $B$ be an $n{\times}l$ matrix of the form $B:=\begin{bmatrix}\vec{b}_1 & \dotsb & \vec{b}_l\end{bmatrix}$. We define the product $AB$, which is an $m{\times}l$ matrix, by:
...
md"""
##### 1-3. Examples of linear transforms on the plane.
Let $A$ be an $m{\times}n$ matrix, let $\vec{x}$ and $\vec{y}$ be $n$-dimensional column vectors respectively, and let $c$ and $d$ be constants. 
The correspondence 

$\vec{x} \mapsto A\vec{x}$

is a mapping of the set $\mathbb{R}^n$ of all the $n$-dimensional column vectors to $\mathbb{R}^m$, and preserves addition and scalar multiplication: 

$A(c\vec{x}+d\vec{y}) = c(A\vec{x}) + d(A\vec{y}).$

Such a mapping is said to be a linear mapping, and in particular a linear transform in case that $m=n$. Let us show two examples of linear transform on $\mathbb{R}^2$ defined by $2\times2$ matrices:
...
md"""
###### 1-3-1. Dilation
Let $\lambda\ne0$ and $\mu\ne0$ be constants. The linear transform  
...
makes the scale of $x$-axis to be $\lambda$, and makes the scale of $y$-axis to be $\mu$. If $\lambda<0$, then the orientation of $x$-axis is opposite to that of $u$-axis. 

Under this linear transformation, the equation $x^2+y^2=1$ of the unit circle with the center at the origin becomes 

$\frac{s^2}{\lambda^2}+\frac{t^2}{\mu^2}=1.$

The following figure describes the case of $\lambda=\sqrt{2}$ and $\mu=1/\sqrt{2}$.
...
md"""
###### 1-3-2. Rotation
Let $\theta \in [0,2\pi)$ be a parameter. The matrix $P(\theta)$ defined by
$P(\theta)
=
\begin{bmatrix}
\cos\theta & -\sin\theta
...
gives the linear transform describing the anti-clockwise rotation of $\theta$ with the center at the origin. We remark that the first and second columns are unit vectors, and that the second column can be written as 
...
which is orthogonal to the first column.

When $\theta=\pi/6$, we draw $x$-axis and $y$-axis in the $uv$-plane:
...
md""" ##### 3-1. An example of singular value decomposition
...
$U$ is a matrix of the clockwise rotation of $\pi/3$, $V^T$ is a matrix of the anti-clockwise rotation of $\pi/4$, and one can understand that $A$ defines a composition mapping of the rotation ($V^T$), the dilation ($\Sigma$), and rotation ($U$). 
...
then the equation $x^2+y^2=1$ of the unit circle becomes a slanted ellipse  $49s^2+15\sqrt{3}st+19t^2=16$. The clockwise rotation gives the directions of the major and minor axes of the ellipse. 
"""
...
md""" ##### 3-2. Singular value decomposition and low rank approximation of matrices
...
git-tree-sha1 = "f7a9573198e876482b28f441c75b3fdc935eb052"
