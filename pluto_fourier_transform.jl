...
The Fourier transform of an intergrable function $f(x)$ on $\mathbb{R}$ is defined by 

$\hat{f}(\xi):=\int_{\mathbb{R}}e^{-2\pi{i}x\xi}f(x)d\xi, \quad \xi\in\mathbb{R}.$

The Fourier transform of a Gaussian function is a Gaussian function:

$\int_{\mathbb{R}}e^{-2\pi{i}x\xi-\pi{x^2}}d\xi=e^{-\pi\xi^2}$

Julia Programming Language computes this integral:
...
begin
	x1 = range(-10, 10, length = 201);
	F=sinc.(x1/s);
	f=ones(201);
	for k=1:100
		f[k]=2*(1-cos(pi*x1[k]))/(pi*x1[k])^2;
		f[k+101]=2*(1-cos(pi*x1[k+101]))/(pi*x1[k+101])^2;
	end
	
	for n=1:20
		a=2*(1-cos(pi*n*s))/(pi*n*s)^2;
        b=sinc.(x1/s-n*ones(201));
        c=sinc.(x1/s+n*ones(201));
        F=F+a*b+a*c;
	end
end

# ╔═╡ 1d325d98-2ea3-4577-a442-79766c114c32
begin
    plot([F,f],
		 grid=false,
		 linewidth=2,
...
# ╔═╡ 61ef730a-274d-4fd6-ac9d-34893792d08a
begin
    N5=100
	KK5=zeros(N5+1,N5+1);
	KK6=zeros(N5+1,N5+1);
...
# ╔═╡ 4de3cbc1-0b58-453c-8957-336a6520dc68
begin
	P5=plot(K5,
		title="Convolution kernel for the Japanese seismic scaling",
		grid=:false,
		xlim=(21,181),
...
git-tree-sha1 = "6d0eb95c02b05b5feb2b291e99d70ad38f7647a9"
